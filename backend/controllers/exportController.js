...
const processMarkdownToDocx = (markdownContent) => {
    ...
    for(let i = 0; i < tokens.length; i++) {
        ...
        try{
                ...
                if(nextToken && nextToken.type === "inline") {
                    ...
                    paragraphs.push(new Paragraph({
                        ...
                        style: {
                            ...
                            size : fontSize * 2,  //doc uses half points
                        }
                    })  );
            ...
            console.error("Error processing markdown to docx:", token.type, tokenError);
            continue; 
        } 
    }
    return paragraphs;
    ...
    try {
        ...
        if (book.coverImage && !book.coverImage.includes("pravatar")) {
            ...
            try {
                ...
                console.error(`Could not embedimage: ${imagePath}`, imgErr);
            }
        }
        ...
        book.chapters.forEach((chapter,index) =>{
            try{
                //page break before eac chapter except first
                if(index>0){
                    sections.push(
                        new Paragraph({
                            text: "",
        ...
        if(!res.headersSent){
            res.status(500).json({ message: "Server error exporting book",
                error: error.message,
            });
        }
    }
