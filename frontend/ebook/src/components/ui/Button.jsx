...
const sizes = {
  ...
  return (
    <button className={`inline-flex items-center justify-center font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap ${variants[variant]} ${sizes[size]} ${className? className : ""}`}
    disabled={isLoading }
    {...props}
    >
        {/* variants[variant] here variant is the prop passed and then variants[whatever prop is passed eh primary or secondary] returns the value from the objec t variants above */}
        {isLoading ? (
